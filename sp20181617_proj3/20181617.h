...
void option5(char*);				//progaddr, loader, run, bp 명령어 처리
int progaddr_(char*);				//progaddr처리
int loader_(char*, char*, char*);	//loader 처리
int run_();							//run처리
int bp_(char*);						//bp처리
void bpprint();						//bp출력
int loaderpass1(char*, int);		//pass1처리
int loaderpass2(char*, int);		//pass2처리
void addestab(char*, int, int);		//estab 추가
int searchestab(char*);				//estab 검색
...
enum instruction { help, dir, quit, history, dump, edit, fill, reset, opcode, opcodelist ,assemble, type, symbol,progaddr, loader,bp, run};	//명령어 열거형
enum reg {A, X, L ,B, S, T, F, PC=8, SW};
char* inst_[] = { "help", "h", "dir", "d", "quit", "q", "history", "hi", "dump", "du", "edit", "e", "fill", "f", "reset","opcode", "opcodelist" ,"assemble", "type", "symbol", "progaddr", "loader", "bp", "run"}; //입력될수 있는 모든 명령어 형태
...
typedef struct NODE_4{
	...
	int address;
	struct NODE_4* next;
}ESTABNODE;						//estab의 node
ESTABNODE* estab[3]={NULL, NULL, NULL};//estab hashtable

int prostart=0;					//프로그램 시작
int csaddr[3]={0, 0,0};			//각 파일별 프로그램 시작
int cslth[3]={0, 0,0};			//각 파일별 프로그램 길이
...
int Sreg;
int Treg;
char conditioncode;				//cc

int bparray[100];				//bp 값
int totallength=0;				//프로그램 총길이
